...
// Inserts a line break at the cursor when the user presses Shift+Enter.
...
const sendMessage = async () => {
  ...
  try {
    ...
    errorMessage.value =
      ...
      "AI is unavailable. Start Ollama and download the selected model.";
  } finally {
...
onMounted(async () => {
  ...
  channel = $supabase
    ...
    .on(
      ...
      (payload) => addMessageIfNew(payload.new as Message),
    )
    .subscribe();
});
...
<template>
  <main class="flex min-h-screen flex-col bg-slate-950 text-slate-100">
    <header
      class="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-5 py-4"
    >
      <div>
    ...
    <section class="mx-auto flex w-full max-w-3xl flex-1 flex-col px-4 py-6">
      <p
        v-if="errorMessage"
        class="mb-4 rounded-lg bg-red-950/60 p-3 text-sm text-red-200"
      >
        {{ errorMessage }}
      </p>
      <div v-if="loading" class="grid flex-1 place-items-center text-slate-400">
        Loading messages…
      </div>
      <div v-else class="flex flex-1 flex-col gap-3 overflow-y-auto pb-5">
        <div v-if="!messages.length" class="my-auto text-center text-slate-500">
        ...
        <article
          ...
          :class="
            message.is_ai
              ? 'bg-violet-950/80 border border-violet-800 self-start'
              : message.user_id === currentUserId
                ? 'bg-emerald-600 text-slate-950 self-end'
        ...
        <p
          v-if="aiThinking"
          id="latest-message"
          class="w-fit rounded-2xl bg-violet-950/80 px-4 py-3 text-sm text-violet-200"
        >
          SimpleChat AI is thinking…
        </p>
      </div>
      <form
        class="sticky bottom-0 rounded-2xl border border-slate-700 bg-slate-900 p-3 shadow-xl"
        @submit.prevent="sendMessage"
      >
        <label
          class="mb-3 flex cursor-pointer items-center gap-2 text-sm text-slate-300"
          ><input v-model="askAi" type="checkbox" class="accent-emerald-400" />
          Ask AI to reply (local Ollama)</label
        >
        <div class="flex gap-2">
          <textarea
            v-model="draft"
            class="min-h-11 flex-1 resize-none rounded-xl bg-slate-800 px-3 py-2 outline-none ring-emerald-400 focus:ring-2"
            rows="1"
            maxlength="2000"
          ...
          /><button
            class="rounded-xl bg-emerald-400 px-5 font-semibold text-slate-950 disabled:opacity-50"
